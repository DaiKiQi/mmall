...
public class OrderServiceImpI implements IOrderService {
    ...
    public ServerResponse<?> createOrder(Integer userId, Integer shippingId) {
        ...
        ServerResponse<List<OrderItem>> cartOrderItems = getCartOrderItems(userId, cartList);
        if (!cartOrderItems.isSuccess()) {
            return cartOrderItems;
        }
        List<OrderItem> orderItemList = cartOrderItems.getData();
    ...
    public ServerResponse<String> cancelOrder(Integer userId, Long orderNo) {
        ...
        if (order.getStatus() != Const.OrderStatusEnum.NO_PAY.getCode()) {
            return ServerResponse.createByErrorMessage("已付款");
        }
    ...
    private List<OrderVo> assembleOrderVoList(List<Order> orderList, Integer userId) {
        ...
        for (Order order : orderList) {
            ...
            if (userId == null) {
                ...
                orderItemList = orderItemMapper.getByOrderNoUserId(generateOrderNo(), userId);
            }
            OrderVo orderVo = assembleOrderVo(order, orderItemList);
    ...
    private ServerResponse<List<OrderItem>> getCartOrderItems(Integer userId, List<Cart> cartList) {
        ...
        for (Cart cart : cartList) {
            ...
            if (Const.ProductStatusEnum.ON_SALE.getCode() != product.getStatus()) {
                return ServerResponse.createByErrorMessage("产品" + product.getName() + "不再售卖");
            }
    ...
    public ServerResponse aliCallback(Map<String, String> params) {
        ...
        Order order = orderMapper.selectByOrderNo(orderNo);
        if (order == null) {
            return ServerResponse.createByErrorMessage("错误订单");
        }
        if (order.getStatus() >= Const.OrderStatusEnum.PAID.getCode()) {
